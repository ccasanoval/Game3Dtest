package com.cesoft.cesgame.RenderUtils
 ...
 * @author jsjolund
 */
//https://github.com/libgdx/libgdx/blob/master/tests/gdx-tests/src/com/badlogic/gdx/tests/bullet/OcclusionCullingTest.java
abstract class OcclusionCuller : Disposable {
	...
	internal val tmpV1 = Vector3()
	internal val tmpV2 = Vector3()
	internal var oclBuffer: OcclusionBuffer? = null

	private val collider = Collider()

	protected inner class Collider : ICollide() {

		/** Callback method for [btDbvt.collideKDOP]. The bounding volume tree node in the parameter
		 * is inside the camera frustum, as are any collision objects it contains.
		 *
		...
		override fun AllLeaves(node: btDbvtNode): Boolean {
			if(node.isleaf()) {
				...
				child = btDbvtNode.internalTemp(nodePointer, false).getChild(0)
				if(child.cPointer != 0L) AllLeaves(child)

				child = btDbvtNode.internalTemp(nodePointer, false).getChild(1)
				if(child.cPointer != 0L) AllLeaves(child)
			}
			return true
		}
		...
		override fun Descent(node: btDbvtNode): Boolean =
			oclBuffer!!.queryAABB(tmpV1.set(node.volume.Center()), tmpV2.set(node.volume.Extents()))
		...
		override fun Process(leaf: btDbvtNode, depth: Float) {
			val obj = leaf.dataAsProxyClientObject
			onObjectVisible(obj)
			val shape = obj.collisionShape
			if(shape is btBoxShape && isOccluder(obj)) {
				oclBuffer!!.drawBB(obj.worldTransform, shape.halfExtentsWithMargin)
			}
	 ...
	 */
	abstract fun isOccluder(obj: btCollisionObject): Boolean
	 ...
	 *
	 * @param obj A collision object which is visible to the camera
	 */
	abstract fun onObjectVisible(obj: btCollisionObject)
	...
	fun performOcclusionCulling(broadphase: btDbvtBroadphase, oclBuffer: OcclusionBuffer, camera: Camera) {
		...
		setFrustumPlanes(camera.frustum)
		btDbvt.collideOCL(broadphase.set1.root, frustumNormals, frustumOffsets, camera.direction, NUM_PLANES, collider)
