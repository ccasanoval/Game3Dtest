...
(
	...
	val bufferHeight: Int) : Disposable {
	/** Depth buffer  */
	...
	// Temporary storage
	private val box = Array(8){Vector3()}
	private val tmpVertices = Array(8){ Quaternion() }
	private val clippedQuad = Array(8){ Quaternion() }
	private val quad = Array(4){ Quaternion() }
	...
	internal class GridPoint3 : com.badlogic.gdx.math.GridPoint3() {
		fun add(other: GridPoint3): GridPoint3 =
			set(x + other.x, y + other.y, z + other.z) as GridPoint3
	}
	...
	private enum class Policy {
		 ...
		 */
		internal fun evaluate(vertices: Array<Quaternion>): Boolean {
			when(this) {
				DRAW -> return false
				QUERY -> {
		 ...
		 */
		internal fun process(depthBuffer: FloatBuffer, bufferIndex: Int, newDepth: Float): Boolean {
			val oldDepth = depthBuffer.get(bufferIndex)
			return when(this) {
				DRAW -> {
	...
	internal class Quaternion : com.badlogic.gdx.math.Quaternion() {
		...
		fun set(quaternion: Quaternion): Quaternion =
			super.set(quaternion) as Quaternion
	...
	fun drawDebugTexture(): TextureRegion? {
		...
		var minDepth = java.lang.Float.POSITIVE_INFINITY
		var maxDepth = java.lang.Float.NEGATIVE_INFINITY
		buffer.clear()
		while(buffer.position() < buffer.capacity()) {
			val depth = MathUtils.clamp(buffer.get(), 0f, java.lang.Float.POSITIVE_INFINITY)
		...
		for(x in 0 until bufferWidth) {
			for(y in 0 until bufferHeight) {
				val depth = MathUtils.clamp(buffer.get(x + y * bufferWidth), 0f, java.lang.Float.POSITIVE_INFINITY)
				val c = depth * extent
	...
	private fun drawTriangle(a: Vector3, b: Vector3, c: Vector3, policy: Policy): Boolean {
		...
		triX.set((a.x * bufferHalfExt.x + bufferOffset.x).toInt(), (b.x * bufferHalfExt.x + bufferOffset.x).toInt(),
				(c.x * bufferHalfExt.x + bufferOffset.x).toInt())
		triY.set((a.y * bufferHalfExt.y + bufferOffset.y).toInt(), (b.y * bufferHalfExt.y + bufferOffset.y).toInt(),
		...
		// Depth interpolation
		val ia = 1f / (triX.x * triY.y - triX.y * triY.x + triX.z * triY.x - triX.x * triY.z + triX.y * triY.z - triX.z * triY.y).toFloat()
